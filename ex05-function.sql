...
SELECT TRIM(' ' FROM ' HelloWorld ') FROM dual;     --공백 자르기
...
*/ 
select MONTHS_BETWEEN(TO_DATE('2016-12-22', 'YYYY-MM-DD'), TO_DATE('2017-05-22', 'YYYY-MM-DD')) from dual;
select ADD_MONTHS(TO_DATE('2022-12-16', 'YYYY-MM-DD'), 1) from dual;
select NEXT_DAY(TO_DATE('2023-08-05', 'YYYY-MM-DD'),7) from dual; --8/5 이후 다음 토요일 날짜 
select LAST_DAY(TO_DATE('2023-08-05', 'YYYY-MM-DD')) from dual;     --고정 날짜
select LAST_DAY(SYSDATE) from dual;     --오늘 날짜
select ROUND(TO_DATE('2023-08-16', 'YYYY-MM-DD'), 'MONTH') from dual;       --15 내림, 16 올림
...
-- 이름을 1-8 글자까지 자르고 '_Us' 붙이고 대문자로 바꿈
...
-- NULLIF() 함수
-- 두 표현식을 비교하여 같으면 null을 반환하고 다르면 expr1을 반환합니다.
...
from employees;
--TO_CHAR(commission_pct) 를 표시 > null이면 TO_CHAR(manager_id) 표시 > ... null 이 아닐때까지 이어간다.
...
from employees;
-- 프로그램 IF문 비슷.
-- select 출력 > case~end 한 컬럼 : job_id가 'IT_PROG'이면 급여 1.10 인상 ..> 나머지는 기본 급여 > 결과를 REVISED_SALARY로 표시
